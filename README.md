...
- [x] What problem does the context API help solve?
  Context api sovle props drilling
- [x] In your own words, describe `actions`, `reducers` and the `store` and their role in Redux. What does each piece do? Why is the store known as a 'single source of truth' in a redux application?
  Action are object that tells the reducer how to change the state. action must contain a type and payload
  Reducers are fucntion that returns state data, it is triggered by an action type reducers are almost like context api
  Store is the object that holds the state data. This is call the single source of truth because everything is stored in this store, which makes it easy to create iniversal apps
- [x] What is the difference between Application state and Component state? When would be a good time to use one over the other?
  Applocation state is global where everything in the application can access, component state is local where the component can only access
- [x] Describe `redux-thunk`, what does it allow us to do? How does it change our `action-creators`?
  Rdux thunk is a middleware that lets you call action creators that returns functions instead of action ohjects
- [x] What is your favorite state management system you've learned and this sprint? Please explain why!
  My fav so far is context api, because it was easeir for me to understand than the redux. 
...
- [x] **RUN** `yarn` to retrieve all `server-side` the dependencies. Did npm install
